...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
  ...
  c.blue = rand() % 256;
	...
	double x_t, y_t, z_t, x_m, y_m, z_m, x_b, y_b, z_b, x_tmp, y_tmp, z_tmp;
	
	x_b = points->m[0][i * 3];
	y_b = points->m[1][i * 3];
	z_b = points->m[2][i * 3];
	x_m = points->m[0][i * 3 + 1];
	y_m = points->m[1][i * 3 + 1];
	z_m = points->m[2][i * 3 + 1];
	x_t = points->m[0][i * 3 + 2];
	y_t = points->m[1][i * 3 + 2];
	z_t = points->m[2][i * 3 + 2];
...
screen s, zbuffer zb, color c) {
	...
	double z_change = (z1 - z0) / ((double) (loop_end - loop_start));
